...
.PHONY: db-up db-down run seed smoke test test-nodb sqlc sqlc-check spec lint web-dev web-test web-build web-api-check features-gen-check e2e e2e-ui
...
e2e: db-up ## Run the Gherkin E2E suite (Playwright boots the API + web client itself; needs Docker)
	cd web && npm run e2e

e2e-ui: db-up ## Same suite in Playwright's interactive UI mode (watch a scenario step through the browser)
...
features-gen-check: ## Fail if generated Playwright specs (.features-gen/) are stale relative to e2e/features/*.feature (what CI runs)
	cd web && npx bddgen
	git diff --exit-code -- web/.features-gen
