...
The typed data-access layer is generated by [sqlc](https://sqlc.dev) (`brew install sqlc`). SQL lives in `internal/infrastructure/postgres/query/*.sql` (one file per aggregate, annotated with `-- name:`); the embedded migrations are the schema. Run `make sqlc` (i.e. `sqlc generate`) after editing any `.sql` file — the generated `internal/infrastructure/postgres/sqlc` package is committed, so regenerate and commit together. Config is `sqlc.yaml`; uuid columns are overridden to `github.com/google/uuid.UUID` (not `pgtype.UUID`) so repositories stay in tally-up's native uuid types.
